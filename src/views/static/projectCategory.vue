<template>
  <div style="margin-top:10px">
    <div style="width: 95%; margin:0 auto">
      <el-form-item
        label="商品分类编号"
        :label-width="110"
        style="display:inline-flex; margin-right: 10px;"
      >
        <el-input
          v-model="searchId"
          placeholder="请输入商品分类编号"
        ></el-input>
      </el-form-item>
      <el-button
        type="primary"
        @click="search"
      >查询</el-button>
      <el-button
        type="warning"
        @click="research"
      >重置</el-button>
      <el-button
        type="success"
        @click="toAdd"
      >添加</el-button>
    </div>
    <el-table
      :data="categories"
      style="width: 95%; margin:0 auto"
    >
      <el-table-column
        prop="id"
        label="编号"
        width="80"
      />
      <el-table-column
        prop="name"
        label="名称"
        width="120"
      />
      <el-table-column
        label="分类级别"
        width="100"
      >
        <template #default="scope">
          {{scope.row.level == 0 ? "1级":"2级"}}
        </template>
      </el-table-column>
      <el-table-column
        label="显示在导航栏"
        width="120"
      >
        <template #default="scope">
          {{scope.row.navStatus == 0 ? "不显示":"显示"}}
        </template>
      </el-table-column>

      <el-table-column
        label="上级编号"
        width="100"
        prop="parentId"
      />

      <el-table-column
        label="显示状态"
        width="100"
      >
        <template #default="scope">
          {{scope.row.showStatus == 0 ? "不显示":"显示"}}
        </template>
      </el-table-column>

      <el-table-column
        label="描述"
        width="120"
        prop="description"
      />

      <el-table-column
        fixed="right"
        label="功能管理"
        width="120"
      >
        <template #default="scope">
          <el-button
            link
            type="primary"
            size="small"
            @click="toEdit(scope.row)"
          >更新</el-button>
          <el-button
            link
            type="primary"
            size="small"
            @click="del(scope.row.id)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
      layout="prev, pager, next"
      :page-size="page.size"
      :total="page.total"
      style="width: 95%; margin:0 auto"
      @current-change="currentchange"
    />
  </div>
  <el-dialog
    v-model="dialogFormVisible"
    title="商品分类编辑"
  >
    <el-form :model="projectCategory">

      <el-form-item
        label="名称"
        :label-width="formLabelWidth"
      >
        <el-input
          v-model="projectCategory.name"
          placeholder="请输入名称"
          style="width:80%;"
        ></el-input>
      </el-form-item>
      <el-form-item
        label="分类等级"
        :label-width="formLabelWidth"
      >
        <el-select
          v-model="projectCategory.level"
          placeholder="请选择分类等级"
          style="width:80%;"
        >
          <el-option
            label="1级"
            :value="0"
          />
          <el-option
            label="2级"
            :value="1"
          />
        </el-select>
      </el-form-item>
      <el-form-item
        label="导航栏"
        :label-width="formLabelWidth"
      >
        <el-select
          v-model="projectCategory.navStatus"
          placeholder="请选择是否显示在导航栏"
          style="width:80%;"
        >
          <el-option
            label="不显示"
            :value="0"
          />
          <el-option
            label="显示"
            :value="1"
          />
        </el-select>
      </el-form-item>
      <el-form-item
        label="上级编号"
        :label-width="formLabelWidth"
      >
        <el-input-number
          v-model="projectCategory.parentId"
          placeholder="请输入上级编号"
          style="width:80%;"
        >
        </el-input-number>
      </el-form-item>
      <el-form-item
        label="显示状态"
        :label-width="formLabelWidth"
      >
        <el-select
          v-model="projectCategory.showStatus"
          placeholder="请选择显示状态"
          style="width:80%;"
        >
          <el-option
            label="不显示"
            :value="0"
          />
          <el-option
            label="显示"
            :value="1"
          />
        </el-select>
      </el-form-item>

      <el-form-item
        label="描述"
        :label-width="formLabelWidth"
      >
        <el-input
          v-model="projectCategory.description"
          placeholder="请输入描述"
          type="textarea"
          :rows="2"
        ></el-input>
      </el-form-item>

    </el-form>
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogFormVisible = false">取消</el-button>
        <el-button
          type="primary"
          @click="save"
        >保存
        </el-button>
      </span>
    </template>
  </el-dialog>
</template>
          
          <script>
import { defineComponent } from "vue";
import {
  updateOne,
  addOne,
  delOne,
  getProjectCategoryPage,
  getOne,
} from "../../http/projectCategory";
import { cloneDeep } from "lodash-es";
export default defineComponent({
  data() {
    return {
      categories: [],
      page: {
        total: 0,
        current: 1,
        size: 10,
      },
      searchId: null,
      dialogFormVisible: false,
      projectCategory: {
        id: 0,
        level: 0,
        name: "",
        navStatus: 0,
        parentId: 0,
        showStatus: 0,
      },
      formLabelWidth: 80,
    };
  },
  mounted() {
    this.getCategoriesPage(1);
  },
  methods: {
    search() {
      if (this.searchId == null || this.searchId == "") {
        alert("请输入商品分类编号后查询");
        return;
      }
      getOne(this.searchId)
        .then((res) => {
          if (res.data == null) {
            this.categories = [];
          } else {
            this.categories = [];
            this.categories.push(res.data.help);
          }
        })
        .catch((err) => {
          console.log(err);
        });
    },
    research() {
      this.searchId = null;
      this.getCategoriesPage(1);
    },
    toEdit(projectCategory) {
      this.dialogFormVisible = true;
      this.projectCategory = cloneDeep(projectCategory);
    },
    getCategoriesPage(current) {
      const data = {
        current: current,
        size: 10,
      };
      getProjectCategoryPage(data)
        .then((res) => {
          console.log(res);
          const page = res.data.page;
          this.categories = page.records;
          this.page = page;
        })
        .catch((err) => {
          console.log(err);
        });
    },
    currentchange(current) {
      // console.log(current);
      this.getCategoriesPage(current);
      this.page.current = current; //数据更新到目前显示的页面
    },
    del(id) {
      if (confirm("是否删除") == true) console.log(id);
      console.log(id);
      delOne(id)
        .then((res) => {
          if (res.success) {
            this.getCategoriesPage(this.page.current);
          } else {
            return false;
          }
        })
        .catch((err) => {});
    },
    toAdd() {
      this.dialogFormVisible = true;
      this.projectCategory = {
        id: 0,
        level: 0,
        name: "",
        navStatus: 0,
        parentId: 0,
        showStatus: 0,
      };
    },
    save() {
      const projectCategory = this.projectCategory;
      if (projectCategory.id == 0) {
        addOne(projectCategory)
          .then((res) => {
            if (res.success) {
              this.dialogFormVisible = false;
              this.getCategoriesPage(this.page.current);
              ElMessage(res.msg);
            } else {
              ElMessage(res.msg);
              return false;
            }
          })
          .catch((err) => {
            ElMessage("网络错误联系管理员");
          });
      } else {
        updateOne(projectCategory)
          .then((res) => {
            if (res.success) {
              this.dialogFormVisible = false;
              this.getCategoriesPage(this.page.current);
              ElMessage(res.msg);
            } else {
              ElMessage(res.msg);
              return false;
            }
          })
          .catch((err) => {
            ElMessage("网络错误联系管理员");
          });
      }
    },
  },
});
</script>
      <style scoped>
.el-col-12 {
  width: 15%;
}
</style>